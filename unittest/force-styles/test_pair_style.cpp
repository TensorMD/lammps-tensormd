                    ...
                    bool ignore_failure = false)
{
    ...
    delete info;
    if (nfail > 0 and not ignore_failure) {
...
TEST(PairStyle, plain)
{
    ...
    if (test_config.pair_style != "rann" && test_config.pair_style != "tensormd") {
        if (!verbose) ::testing::internal::CaptureStdout();
...
TEST(PairStyle, omp)
{
    ...
    ::testing::internal::CaptureStdout();
    bool ignore_failure = false;
    if (test_config.pair_style == "tensormd") {
        ignore_failure = true;
    }
    LAMMPS *lmp = init_lammps(args, test_config, true, ignore_failure);
