...
void TensorMDTimer::print()
{
  ...
  if (total_flops >= 1e21) {
    unit = 'Z';
    scale = 1e21;
  } else if (total_flops >= 1e18) {
    unit = 'E';
    scale = 1e18;
  } else if (total_flops >= 1e15) {
    unit = 'P';
    scale = 1e15;
  ...
  for (const auto &routine : timer) {
    ...
    if (routine.second.secs < 1e-6)
      FLOPS = 0.0;
    else
      FLOPS = routine.second.flops / routine.second.secs / scale;
    sprintf(buf, "%14s: %10.4f %cFlops, %12.4f secs, %12.4f %cFLOPS", key,
            routine.second.flops / scale, unit, routine.second.secs, FLOPS,
            unit);
  ...
  if (total_secs < 1e-6)
    ...
    FLOPS = total_flops / total_secs;
  sprintf(buf, "%14s: %10.4f %cFlops, %12.4f secs, %12.4f %cFLOP/s", "Overall",
          total_flops, unit, total_secs, FLOPS, unit);
  if (logfile) {
    fprintf(logfile,
            "------------------------------------------------------------------"
            "--------\n");
...
{
  timer[TIMER::V].flops += a * b * (d * c * k * 2);
