...
template <typename Scalar> class TDNP {
 ...
 public:
  ...
  // The default functions: atomic temperature is disabled
  void compute(int elti, Scalar *x_in, int n, Scalar *dfdx, Scalar etemp,
               TDPE<Scalar> &pe);
  void forward(int elti, Scalar *x_in, int n, Scalar etemp, TDPE<Scalar> &pe);
  void backward(int elti, Scalar etemp, Scalar *grad_in, Scalar *grad_out);

  // Atomic temperature is enabled
  void compute(int elti, Scalar *x_in, int n, Scalar *dfdx, Scalar *etemp,
               TDPE<Scalar> &pe);
  void forward(int elti, Scalar *x_in, int n, Scalar *etemp, TDPE<Scalar> &pe);
  void backward(int elti, Scalar *etemp, Scalar *grad_in, Scalar *grad_out);
