...
template <typename Scalar>
void NeuralNetworkPotential<Scalar>::compute(int elti, Scalar *x_in, int n,
                                             PE<Scalar> &pe, Scalar *dydx)
{
  forward(elti, x_in, n, pe);
...
template <typename Scalar>
void NeuralNetworkPotential<Scalar>::forward(int elti, Scalar *x_in, int n,
                                             PE<Scalar> &pe)
{
  if (use_resnet_dt || actfn == Softplus)
    pe.toten = forward_resnet(elti, x_in, n, pe.eatom);
  else
    // Computing gradients of softplus(x) will be significantly slower for this
    // implementation.
    pe.toten = forward_dense(elti, x_in, n, pe.eatom);
}
