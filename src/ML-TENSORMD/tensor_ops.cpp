...
template <typename Scalar>
void kernel_F1(TensorMap<Tensor<Scalar, 4>> &U, TensorMap<Tensor<Scalar, 4>> &dHdr,
               ...
               TensorMap<Tensor<Scalar, 4>> *F1)
{
  ...
  Scalar y;
  athread_tensoralloy_batch_ddot_F1(amax * bmax * cmax, kmax, 3, F1->data(),
                                       U.data(), dHdr.data(), drdrx.data());
}
               ...
               TensorMap<Tensor<Scalar, 4>> *F2)
{
  ...
  athread_tensoralloy_batch_dgemv_F2(amax * bmax * cmax, 3, dmax, dMdrx.data(),
                                        V.data(), F2->data());
}
              ...
              TensorMap<Tensor<Scalar, 4>> *U)
{
  ...
  athread_tensoralloy_batch_small_dgemm(limit, c, k, d,
                                           0, 0, M.data(),
                                           dEdS.data(), U->data());
}
              ...
              TensorMap<Tensor<Scalar, 4>> *V)
{
  ...
  const int k = static_cast<int>(dEdS.dimension(1));    // gemm K
  athread_tensoralloy_batch_small_dgemm(limit, c, d, k,
                                           0, 1, H.data(),
                                           dEdS.data(), V->data());
}
              ...
              TensorMap<Tensor<Scalar, 4>> *P)
{
  ...
  athread_tensoralloy_batch_small_dgemm(limit, k, d, c,
                                           1, 1, H.data(),
                                           M.data(), P->data());
}
                 ...
                 TensorMap<Tensor<Scalar, 4>> *dEdS)
{
  ...
  athread_tensoralloy_dEdS_P(a * b * k, m, dEdS->data(), dEdG.data(),
                                       P.data());
}
                 ...
                 TensorMap<Tensor<Scalar, 4>> *dEdG)
{
  ...
  athread_tensoralloy_dEdG(size / m, m, G.data(), dEdG->data());
}
