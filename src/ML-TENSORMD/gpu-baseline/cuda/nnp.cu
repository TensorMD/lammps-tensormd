...
__global__ void relu_device(Scalar *x, int n, Scalar *dx)
{
...
__global__ void softplus_device(double *x, int n, double *dx)
{
  double exp_;
  int i = threadIdx.x + blockDim.x * blockIdx.x;
  if (i < n) {
    ...
    if (x[i] < -threshold) { x[i] = log1p(exp_); }
    dx[i] = 1.0 / (1.0 + 1.0 / exp_);
  }
}

__global__ void softplus_device(float *x, int n, float *dx)
{
  float exp_;
  int i = threadIdx.x + blockDim.x * blockIdx.x;
  if (i < n) {
...
template <typename Scalar>
__global__ void squareplus_device(Scalar *x, Scalar *dx, int n)
{
  int i = threadIdx.x + blockDim.x * blockIdx.x;
  Scalar xi;
  Scalar z;
                   ...
                   Scalar *biases, Scalar **dz, int nlocal_old, bool DToH)
{
  ...
  for (i = 0; i < nlayers; i++) {
    ...
    if (i < nlayers - 1) {
      if (actfn == 3) {
        squareplus_device<<<(M * N + maxThread - 1) / maxThread, maxThread>>>(
            C, (*dz) + i * n * max_size_dz, M * N);
      } else if (actfn == 1) {
        softplus_device<<<(M * N + maxThread - 1) / maxThread, maxThread>>>(
            C, M * N, (*dz) + i * n * max_size_dz);
      }
    } else {
      break;
    }
    if (i > 0 && use_resnet_dt && K == N)
      gpublasaxpy(handle, M * N, &alpha, x, 1, C, 1);
    if (C_is_pool_0) {
      x = pool[0];
      C = pool[1];
    } else {
