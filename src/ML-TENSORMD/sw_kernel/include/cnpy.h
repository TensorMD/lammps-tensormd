...
struct NpyArray {
  NpyArray(std::vector<size_t> _shape, size_t _word_size,
      ...
      word_size(_word_size), fortran_order(_fortran_order)
  {
    num_vals = 1;
  ...
  {
    auto flat = new T[num_vals];
    memcpy(flat, reinterpret_cast<T *>(&(*data_holder)[0]),
           data_holder->size());
    return flat;
  }
...
void npy_save(const std::string &fname, const T *data,
              const std::vector<size_t> &shape, const std::string &mode = "w")
{
  FILE *fp = nullptr;
  std::vector<size_t>
              ...
              const std::vector<size_t> &shape, const std::string &mode = "w")
{
  ...
  if (fp) {
    ...
    // header then read and store the global header.
  ...
  footer +=
      (uint32_t) (global_header_offset + nbytes +
                  local_header
                      .size());    //offset of start of global headers, since
                                   // global header now starts after newly
                                   // written array
