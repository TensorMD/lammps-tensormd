...
ComputeTensorMDDensity::ComputeTensorMDDensity(LAMMPS *lmp, int narg,
                                               char **arg) :
...
void ComputeTensorMDDensity::init()
{
  ...
  if (modify->get_compute_by_style("tensormd/density").size() > 1 &&
      comm->me == 0)
...
void ComputeTensorMDDensity::read_potential_file(const std::string &globalfile)
{
  ...
  auto it = npz.find("precision");
  if (it != npz.end()) {
    if (it->second.data<int>()[0] == 32) {
      fpot = new TENSORMD::TensorMD<float>(memory, error, screen, logfile,
                                           comm->me);
      fpot->timer_switch(false);
    }
  }
  if (!fpot) {
    dpot = new TENSORMD::TensorMD<double>(memory, error, screen, logfile,
                                          comm->me);
...
void ComputeTensorMDDensity::compute_peratom()
{
  ...
  if (dpot) {
    ...
    dpot->setup_tensors(list->ilist, atom->type, map, atom->x, list->numneigh,
                        list->firstneigh);
    dpot->compute_radial_density();
    dpot->compute_projected_density();
  } else {
    fpot->setup_local(atom->nlocal, nnl_max, typenums);
    fpot->setup_tensors(list->ilist, atom->type, map, atom->x, list->numneigh,
                        list->firstneigh);
